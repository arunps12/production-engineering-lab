...
└── curriculum/            ← Production engineering exercises
    ...
    ├── 02-git-version-control/    (3 files — branching, hooks, workflows)
    ├── 03-networking/             (8 files — TCP/IP, DNS, sockets, ports)
    ├── 04-database/               (4 files — SQL, Redis, migrations, Docker)
    ├── 05-rest-api/               (4 files — HTTP methods, Pydantic, OpenAPI)
    ├── 06-fastapi/                (11 files — endpoints, middleware, testing, DI)
    │   └── E_complete_api/            Full production API scaffold
    ├── 07-rest-api-crud-labs/     (11 files — CRUD API, PUT/PATCH, tests)
    ├── 08-postgresql-production-labs/ (12 files — Docker PG, schema, EXPLAIN, Alembic)
    ├── 09-docker/                 (6 files — Dockerfile, compose, debugging)
    ├── 10-docker-debug-labs/      (12 files — broken compose, debug checklist)
    ├── 11-security/               (3 files — JWT, RBAC, secrets, validation)
    ├── 12-nginx/                  (5 files — config, proxy, load balancing)
    ├── 13-cicd/                   (4 files — GitHub Actions, pipelines)
    ├── 14-ci-cd-practice/         (3 files — GitHub Actions, smoke tests)
    ├── 15-monitoring/             (4 files — logging, Prometheus, Grafana)
    ├── 16-cloud-infrastructure/   (4 files — Terraform, health checks, IaC)
    ├── 17-ansible-practice/       (6 files — playbooks, templates, inventory)
    ├── 18-elasticsearch-practice/ (9 files — ES Docker, Query DSL, reindex)
    ├── 19-rdf-sparql-labs/        (16 files — Fuseki, SPARQL, SQL comparison)
    └── 20-capstone/               (27 files — complete production service)
